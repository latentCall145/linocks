...
while true; do
	...
	case $id in
		# 11 digit id = video
		#???????????) mpv "$videolink$id";;
                #???????????) mpv --ytdl-format="(bestvideo+bestaudio)[ext=webm]/bestvideo+bestaudio/best/bestvideo+bestaudio/best" "$videolink$id";;
                ???????????) mpv --ytdl-format="(bestvideo+bestaudio)/bestvideo+bestaudio/best/bestvideo+bestaudio/best" "$videolink$id";;
